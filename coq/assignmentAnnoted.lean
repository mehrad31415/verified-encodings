...
-- (λ _, tt) is a lambda fn. sets all vars in assignment to true
--There exists a default element of type assignment V, namely the function that maps every variable to true
--important because that means the type (assignment V) is nonempty (because default assignment exists)
...
--establishes variables that will be used throughout this section
--for variables tau_1, tau_2, tau_3, in assignment V, 
--s, s_1, s_2, in finset V are sets of vars
-- v : V is a variable, l is a literal
...
variable [decidable_eq V] /-says that equality of V is decidable-/
...
/-
If
τ₁ and τ₂ agree on a set s, and
τ₁ and τ₂ agree at a specific variable v
Then they agree on the union {v} ∪ s, where
{v} is the singleton set with the var v
-/
/-
hagree_on is the original agreement,
heq is the equality of v,
u is an arbitrary variable
hu : x in {v} U s says x is in the union set-/\

/-
or.elim (mem_union.mp hu) splits up into
case where x is in {v} and where x is in s
-/
  (λ h, (mem_singleton.mp h).symm ▸ heq) 
  /-c1: If u=v, uses heq to show  τ₁u =τ₂u-/
  (λ h, hagree_on u h)
  /-c2: u in set s, uses hagree_on-/
...
/-(mem_inter.mp hv).1 means to destruct
into 2 cases, from x in A intersection B to x in A and x in B
to just the 1st case-/
